namespace ContactListUnitTest
{
    ...
    public abstract class Mock<T> : Stub<T> where T : class
    {
        ...
        {
            if (this.memberInvocationVerifiers.Count <= 0) Assert.True(false, "There is not any verification to check");
    ...
    public abstract class Spy<T> : MockBase<T> where T : class
    {
        ...
        protected Spy() : base(MockBehavior.Strict)
        {
        ...
        {
            Assert.True(this.memberInvocations.Any(m => m.Name == methodName), "Checking '" + methodName + "' has been called");
        }

        internal void Called(int counter, string methodName)
        {
            Assert.True(counter == this.memberInvocations.Count(m => m.Name == methodName), "Checking '" + methodName + "' has been called " + counter + " times");
        }
